...
export const CreateRoomForm = ({ onClose }: CreateRoomFormProps) => {
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (data: CreateRoomFormData) => {
    const res = await authApi.post("/room/create", data);
    console.log("[CreateRoom] API response:", res.data);
    const { roomId, name } = res.data;
    navigate(`/room/${roomId}`, { state: { roomName: name } });
  };
    ...
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col gap-1 text-center">
          ...
          <p className="text-white/45 text-sm">Give your room a name to get started</p>
        </div>
